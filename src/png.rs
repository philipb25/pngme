...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk, RemoveFirstChunkError> {
        ...
        let index = self
            ...
            .ok_or(RemoveFirstChunkError::NotFound)?;
...
#[derive(Debug)]
#[non_exhaustive]
pub enum RemoveFirstChunkError {
    NotFound,
    ParseChunk { source: ParseChunkTypeError },
}

impl Display for RemoveFirstChunkError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RemoveFirstChunkError::NotFound => f.write_str("chunk type not found"),
            RemoveFirstChunkError::ParseChunk { .. } => {
                f.write_str("unable to parse as `ChunkType`")
            }
        }
    }
}

impl Error for RemoveFirstChunkError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            RemoveFirstChunkError::NotFound => None,
            RemoveFirstChunkError::ParseChunk { source } => Some(source),
        }
    }
}
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let types = self
            ...
            .collect::<Vec<_>>();
        f.debug_struct("Png").field("chunks", &types).finish()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        let mut signature_buf = [0u8; Png::SIGNATURE.len()];
        reader
            .read_exact(&mut signature_buf)
            .map_err(|err| TryFromSliceError {
                kind: TryFromSliceErrorKind::Read(err),
            })?;

        if Png::SIGNATURE != signature_buf {
            return Err(TryFromSliceError {
                kind: TryFromSliceErrorKind::InvalidHeader,
            });
        }

        let mut chunks: Vec<Chunk> = Vec::new();
        'chunks: loop {
            match Chunk::try_from(reader) {
                Ok(chunk) => {
                ...
                Err(err) => Err(TryFromSliceError {
                    kind: TryFromSliceErrorKind::Chunk(err),
                })?,
            }
        }
...
impl Display for TryFromSliceError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("unable to convert bytes to `Png`")
    }
}

#[derive(Debug)]
#[non_exhaustive]
pub enum TryFromSliceErrorKind {
    Chunk(chunk::TryFromBytesError),
    InvalidHeader,
    Read(io::Error),
}
...
impl Display for TryFromSliceErrorKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TryFromSliceErrorKind::InvalidHeader => f.write_str("invalid file signature"),
            TryFromSliceErrorKind::Read(err) => Display::fmt(err, f),
        }
    }
}

impl Error for TryFromSliceErrorKind {}
